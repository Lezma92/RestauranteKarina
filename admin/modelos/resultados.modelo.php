...
class ModeloResultados
{
	...
	static public function mdlQuinielasxJornada($id_usuario, $id_joranda,$status)
	{
		...
		q.id_acceso = :idacceso AND j.id = :id_joranda and q.status_pago = :stats	
		WHERE DATE_ADD(q.fecha_registro,INTERVAL 10 DAY) > CURRENT_TIMESTAMP()
		...
		$conexion->bindParam("stats", $status, PDO::PARAM_STR);
		if ($conexion->execute()) {
			return $conexion->fetchAll();
		}
	}
